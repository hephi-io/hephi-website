...
const ProcessSection = () => {
  ...
  return (
    <section className="mb-12 sm:mb-[120px]">
      <div className="w-[90%] mx-auto sm:w-[94.24%] lg:max-w-[1391px] lg:flex lg:justify-between lg:items-start">
        <Heading
          as="div"
          className="text-base leading-[19.2px] text-[#2B2B2B] dark:text-white mb-10 sm:text-xl sm:leading-6"
        >
          PROCESS
        </Heading>
        <div className="lg:w-[56.22%]">
          <div className="flex flex-col gap-y-6 sm:grid sm:grid-cols-2 sm:gap-6">
            {processSectionCards.map((processSectionCard, index) => (
              <div
                key={index}
                className="relative rounded-3xl bg-[#F3F3F3] dark:bg-[#1B1B1C] dark:border dark:border-[#313131] py-9"
              >
                <div className="w-[90%] h-[252px] mx-auto flex flex-col justify-between sm:w-[81.10%] sm:h-[368px] lg:w-[83.92%]">
                  <div className="w-9 h-9 rounded-full border border-[#363636] dark:border-[#D1D1D2] flex justify-center items-center text-base leading-4 text-[#363636] dark:text-[#D1D1D2] z-10">
                    {processSectionCard.number}
                  </div>
                  <div>
                    <Heading
                      as="div"
                      className="text-2xl leading-6 text-[#363636] dark:text-white sm:text-[28px] sm:leading-7 lg:text-4xl lg:leading-9"
                    >
                      {processSectionCard.header}
                    </Heading>
                    <div className="text-base leading-4 text-[#363636] dark:text-[#D1D1D2] sm:text-xl sm:leading-5 mt-6">
                      {processSectionCard.paragraphText}
                    </div>
                  </div>
                </div>
                <div className="absolute left-[25%] bottom-[53%] w-[44.17%] h-[200px] sm:left-[12.5%] sm:bottom-[47.5%] sm:w-[75.59%] sm:h-[286px]">
                  <Image
